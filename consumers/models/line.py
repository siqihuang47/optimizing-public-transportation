...
class Line:
    ...
    def process_message(self, message):
        ...
        if message_topic == "faust_transformed_stations": # Process messages for stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message_topic.startswith("station.arrivals"): # Process messages for arrival topic
            self._handle_arrival(message)
        elif message_topic == 'TURNSTILE_SUMMARY': # Process message for KSQL Turnstile Summary Topic
